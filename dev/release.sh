...
if [ "${RELEASE_CHECK_ORIGIN:-yes}" = "yes" ]; then
  ...
  if [ "${git_origin_url}" != "git@github.com:enactic/openarm_driver.git" ]; then
    echo "This script must be ran with working copy of enactic/openarm_driver."
...
if [ "${RELEASE_TAG:-yes}" = "yes" ]; then
  echo "Tag"
  git tag -a -m "OpenArm Driver ${version}" "${version}"
